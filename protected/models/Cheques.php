...
class Cheques extends CustomCActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('id, operacionChequeId, tasaDescuento, clearing, pesificacion, numeroCheque, libradorId, bancoId, montoOrigen, fechaPago, tipoCheque, endosante, montoNeto, estado, userStamp, timeStamp, sucursalId, fechaIni, fechaFin, librador_denominacion, tieneNota,fechaCompra', 'safe', 'on' => 'search'),
        );
    }
...
//        $criteria->select = 't.*';
     ...
     public function searchCompradosFecha() {
        $criteria = new CDbCriteria;
        $criteria->with = array(
            'banco',
            'librador',
        ...
        if ($this->fechaCompra=='null'){
            $this->fechaCompra=date("Y-m-d");
        }else{
            $this->fechaCompra=Utilities::MysqlDateFormat($this->fechaCompra);
        }
        $criteria->compare('operacionCheque.fecha', $this->fechaCompra);
    ...
    public function setFechaFin($value) {
     ...
     public function searchChequesByEstadoByFecha($estado, $chequesId=array(),$fechaIni,$fechaFin) {
        //filtra ademas los cheques que vencen en la ultima semana
...
//        $criteria->compare('denominacion', $this->librador,TRUE);
    ...
    public function getChequesEntregaDevolucion($estado, $clienteId) {
        $chequesColocados = $this->searchChequesByEstado($estado,false);
        ##filtro ahora segun el porcentaje de tenencia que tiene que ser 100%
        //$data = $chequesColocados->getData();
        ...
        $criteria->addCondition("colocacion.estado=1");
        $detalleColocaciones = DetalleColocaciones::model()->findAll($criteria);

        foreach ($detalleColocaciones as $dc) {
          $porcentajeTenencia=round($dc->colocacion->cheque->montoOrigen/$dc->monto)*100;
          if (($porcentajeTenencia==100) and ($dc->colocacion->cheque->estado==$estado)){

             $cheques[]=$dc->colocacion->cheque;
          }

                 //echo "Porcentaje:".$porcentajeTenencia;
        }
        /*
        for($i=0;$i<count($data);$i++) {
            $cheque = $data[$i];
            $porcentajeTenencia = Clientes::model()->getPorcentajeTenencia($cheque->id,$clienteId);
            echo $porcentajeTenencia;
            if($porcentajeTenencia ==100) {
                $cheques[]=$cheque;
            }   
        }
            
        
        //print_r($chequeColocados);*/
