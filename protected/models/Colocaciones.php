...
class Colocaciones extends CustomCActiveRecord {
    ...
    public function inversaValorActual($valorActual, $fechaFin, $tasa, $clearing) {
        ...
        $i = $tasa / 100;
        $divisor = 1 + (($i * $n) / 365);
        $montoOrigen = Utilities::redondear_dos_decimal($divisor * $valorActual);
        return $montoOrigen;
    }
    ...
    public function getColocacionesCliente($clienteId) {
        ...
        $criteria = new CDbCriteria;
        $criteria->with = array( 'detalleColocaciones','cheque' );
        $criteria->compare('clienteId', $clienteId);
        $criteria->compare('t.estado', Colocaciones::ESTADO_ACTIVA);
        $criteria->order="cheque.fechaPago ASC";
    ...
    public function getClearing(){
        ...
        $fechaInicio=Utilities::ViewDateFormat($this->fecha);
