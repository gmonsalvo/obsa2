...
class LibradoresController extends Controller {
    ...
    public function getRanking($paginacion){
        ...
        $criteria = new CDbCriteria();
        $criteria->select = "t.id, SUM(t.montoOrigen) as total, librador.*";
        $criteria->with = array("librador");
        $criteria->condition = "t.estado = ".Cheques::TYPE_EN_CARTERA_COLOCADO;
        $criteria->group = "librador.id";
        $criteria->order = "total DESC";

        $libradores = Cheques::model()->findAll($criteria);
        ...
        if ($totalEnCartera != 0) {
            foreach ($libradores as $librador) {
                $total = $librador->total;
                $porcentajeEnCartera = ($total / $totalEnCartera) * 100;
                $rawData[] = array('id' => $i, 'librador' => $librador->librador->denominacion, 'total' => $total, 'porcentajeEnCartera' => $porcentajeEnCartera);
                $i++;
            }
        }
