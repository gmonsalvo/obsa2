		...
		// $this->performAjaxValidation($model);
			if(isset($_POST['DetallePesificaciones']))
			{
				try {
					...
					if($model->conceptoId == 3) {
						$model->conceptoId = 6;
						$cheque = new Cheques;
						$cheque->attributes=$_POST["Cheques"];
						$cheque->estado = Cheques::TYPE_ACREDITADO;
						$busqueda = $cheque->search();
						if(count($busqueda->getData()) == 0) {
							$cheque->estado = Cheques::TYPE_EN_PESIFICADOR;
							$busqueda = $cheque->search();
							if(count($busqueda->getData()) == 0) {
								$model->addError('error', 'No se encontro ningun cheque con los parametros ingresados');
								throw new Exception("error", 1);	
							}	
						}
						$cheques = $busqueda->getData();
						$cheque=$cheques[0];
						$saldoActual+=$cheque->montoOrigen;

						// hacer movimiento para cargar en el detalle el monto del cheque
						$detallePesificacionCheque = new DetallePesificaciones;
						$detallePesificacionCheque->conceptoId = 3;
						$detallePesificacionCheque->pesificacionId = $pesificacion->id;
						$detallePesificacionCheque->chequeId = $cheque->id;
						$detallePesificacionCheque->monto = $cheque->montoOrigen;
					    $detallePesificacionCheque->estado=DetallePesificaciones::ESTADO_PENDIENTE;
						$detallePesificacionCheque->tipoMov=DetallePesificaciones::TYPE_CREDITO;
						if(!$detallePesificacionCheque->save()) {
							$model->addError('error', 'No se pudo guardar el detalle');
							throw new Exception("error", 1);
						}
					}
					if($saldoActual>0){
